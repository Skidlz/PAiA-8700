...
STOR	sta	PBUF	;SAVE AGAIN
 		rts			;AND RETURN
;
;PROGRAM TRANSPOE MODE - NOTE PLAYED
...
;
	org $1100
STBL
		db	lo REST, lo REST, lo REST, lo REST
...
;
COM		lda STUS	;CHECK OPTIONS
 		pha			;SAVE A COPY
 		asl	a		;METRONOME ON ?
 		bpl	COM0	;NO - BRANCH
 		dec	MTRC	;DECREMENT METRONOME COUNTER
		bpl	COM0	;NOT <0 YET, BRANCH
		ldx	TPO		;TIME UP, GET TEMPO VALUE
		dex			;DECREMENT ONCE
		stx	MTRC	;THEN SAVE AS COUNTER
		lda	#$80	;TO DETERMINE ALTERNATE DISPLAY
		tax			;CYCLE AND "PENDULUM" LEFT
		clc			;PREPARE FOR ADDITTION
		adc	METF	;ADD FLIP-FLOP VALUE
		sta	METF	;SAVE NEW VALUE
		bpl	MET1	;ALTERNATE? - DISPLAY
		lda	PBUF	;OTHERWISE, GET OUTPUT
		ora	#$80	;SET D7
...
MSV1	bcs	MS1		;FIRST PASS?
		...
		sta	MEND	;ZERO M-SEQ POINTER
		sta	TRNS	;ZERO TRANSPOSE
		sta	PBUF	;ZERO OUTPUT NOTE
MS1		lda	CNTR	;GET TIME SINCE LAST NOTE
		ldx	MEND	;AND M-SEQ END POINTER
...
LP1		sta	TPNT	;ZERO T-SEQ POINTER
LP2		sta	MPNT	;AND M-SEQ POINTER
		sta	CNTR	;AND CLOCK (TEMPO CONTER)
CONT	lda	CNTR	;GET CLOCK
		ldy	TPNT	;GET T-SEQ POINTER
		ldx	MPNT	;GET M-SEQ POINTER
		cmp	MTBL2,x	;TIME UP?
		bcc	PL1		;NO, BRANCH
		...
		sty	TPNT	;NO-SAVE T-SEQ POINTER
...
TOUT	jsr	STTP	;SET UP FOR TRANSFER
		lda	MEND	;SAVE M-SEQ END WITH
		sta	MTBL	;M&T-SEQUENCE
		lda	TEND	;ALSO T-SEQUENCE END
		sta	MTBL1
...
;
TCH		sta	MTRC	;ZERO METRONOME CLOCK
		ror	TPO		;HALVE TEMPO VALUE
		bcc	TCHR	;IF NOT ZERO, LEAVE
		ror	TPO		;ZERO, MAKE NOT ZERO
