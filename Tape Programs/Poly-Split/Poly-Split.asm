...
;
	org	$0000
;
STAR	lda	OUTT	;GET THE # OF RESERVED LOW CHANS
		sta	OUT2	;SAVE PERMANENTLY
POSP	ldx	#$07	;SET UP A POINTER/COUNTER
SLP9	lda	#$00	;AND GET READY TO ZERO STUFF
...
SLP6	jsr	NOTE	;MUS 1.0 - REFRESH AND READ AGO KBD
...
;
SPLI	ldy	#$07	;SET UP POINTER TO HIGH BUFFER
		ldx	#$07	;AND ONE TO INPUT BY
SLP0	lda	KTBL,x	;GET THE KEY
...
;
SNX1	lda	OUT2	;GET THE NUMBER OF LOW CHANS AVAILABLE
...
SPL1	lda	HKEY,x	;GET THE HIGH KEY FROM TEMP BUFFER
...
SNX2	dex			;ONE LESS CHANNEL, POINT TO NEXT
...
SLP2	dey			;ONE LESS LOW CHANNEL
		bmi	SNX3	;AND IF ALL DONE, SKIP NEXT TRANSFER
		lda	HKEY,x	;GET THE REAL CHANNEL STATE
		sta	NTBL,x	;PLACE IN OUTPUT BUFFER
SNX3	lda	#$00	;NOW GET READY AND
